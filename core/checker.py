...
from utils import MultPath, remlist_all
from utils.paintext import *


#=========== PATH ===========
PARENT_PATH = Path(__file__).parent
DATA_PATH = PARENT_PATH / "data.csv"
TEMP_PATH = PARENT_PATH / "tmp.csv"
...
class FileChecker() :
    def __init__(self, paths: list[Path]):
        try :
            ...
            if MultPath(paths + [DATA_PATH, TEMP_PATH, BACKUP_PATH]) :
            ...
            for indx, i in enumerate(paths) :
                paths[indx] = str(i)
            if all(os.access(i, os.W_OK | os.R_OK) for i in [DATA_PATH]) :
...
"""
    ...
    def process(self) -> None :
        ...
        with open(DATA_PATH, 'r') as source, open(TEMP_PATH, 'w') as temp, open(PARENT_PATH / "log.txt", 'a') as logger :
